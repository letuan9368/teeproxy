...
# Example:
#   bash Tee-Proxy-Socks5-IP-Port.sh 2000 22000
...
WORKDIR="/home/bkns"
WORKDATA="${WORKDIR}/data_socks5_ip_port.txt"
PROXY_BIN="/usr/local/etc/3proxy/bin/3proxy"
PROXY_CFG="/usr/local/etc/3proxy/3proxy-socks5-ip-port.cfg"
PROXY_SERVICE="/etc/systemd/system/3proxy-socks5-ip-port.service"
...
gen_if_script() {
  local iface="$1"
  cat > "${WORKDIR}/boot_ifconfig_socks5_ip_port.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  chmod +x "${WORKDIR}/boot_ifconfig_socks5_ip_port.sh"
}
...
gen_proxy_list() {
  awk -F "/" '{print $1 ":" $2}' "${WORKDATA}" > "${WORKDIR}/proxy_socks5_ip_port.txt"
}

create_systemd_service() {
  cat > "${PROXY_SERVICE}" <<EOF
[Unit]
Description=3proxy SOCKS5 IP:PORT service
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStartPre=/bin/bash ${WORKDIR}/boot_ifconfig_socks5_ip_port.sh
...
cleanup_old_state() {
  local iface="$1"
  echo "[*] Cleaning old SOCKS5 IP:PORT proxy state..."

  systemctl stop 3proxy-socks5-ip-port >/dev/null 2>&1 || true
  ...
  rm -f "${WORKDATA}" \
        "${WORKDIR}/proxy_socks5_ip_port.txt" \
        "${WORKDIR}/boot_ifconfig_socks5_ip_port.sh" \
        "${PROXY_CFG}"
}
...
main() {
  ...
  echo "[*] Restarting 3proxy-socks5-ip-port..."
  systemctl daemon-reload
  systemctl enable --now 3proxy-socks5-ip-port

  echo "[*] Done."
  echo "[*] Proxy list: ${WORKDIR}/proxy_socks5_ip_port.txt"
  echo "[*] Total proxies: $(wc -l < "${WORKDIR}/proxy_socks5_ip_port.txt")"
}
