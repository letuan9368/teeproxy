...
PROXY_SERVICE="/etc/systemd/system/3proxy-custom.service"
...
[Unit]
Description=3proxy custom service
...
main() {
  ...
  echo "[*] Restarting 3proxy cleanly..."
  systemctl stop 3proxy-custom >/dev/null 2>&1 || true
  pkill -f "${PROXY_BIN}" >/dev/null 2>&1 || true
  systemctl daemon-reload
  systemctl enable --now 3proxy-custom

  echo "[*] Done."
  echo "[*] Proxy list: ${WORKDIR}/proxy.txt"
  echo "[*] Total proxies: $(wc -l < "${WORKDIR}/proxy.txt")"
}
