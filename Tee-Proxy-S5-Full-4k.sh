...
# Example:
#   bash Tee-Proxy-S5-Full-4k.sh 4000 32000 37000
...
WORKDIR="/home/bkns"
WORKDATA_PASS="${WORKDIR}/data_s5_full4k_pass.txt"
WORKDATA_IP_PORT="${WORKDIR}/data_s5_full4k_ip_port.txt"
BOOT_SCRIPT="${WORKDIR}/boot_ifconfig_s5_full4k.sh"

PROXY_BIN="/usr/local/etc/3proxy/bin/3proxy"
PROXY_CFG_PASS="/usr/local/etc/3proxy/3proxy-s5-full4k-pass.cfg"
PROXY_CFG_IP_PORT="/usr/local/etc/3proxy/3proxy-s5-full4k-ip-port.cfg"

SERVICE_PASS="/etc/systemd/system/3proxy-s5-full4k-pass.service"
SERVICE_IP_PORT="/etc/systemd/system/3proxy-s5-full4k-ip-port.service"
...
gen_proxy_lists() {
  awk -F "/" '{print $3 ":" $4 ":" $1 ":" $2}' "${WORKDATA_PASS}" > "${WORKDIR}/proxy_s5_full4k_pass.txt"
  awk -F "/" '{print $1 ":" $2}' "${WORKDATA_IP_PORT}" > "${WORKDIR}/proxy_s5_full4k_ip_port.txt"
}

create_services() {
  cat > "${SERVICE_PASS}" <<EOF
[Unit]
Description=3proxy s5 full4k auth
...
[Unit]
Description=3proxy s5 full4k no-auth
After=network-online.target
Wants=network-online.target
...
cleanup_old_state() {
  local iface="$1"
  echo "[*] Cleaning old s5 full4k state..."

  systemctl stop 3proxy-s5-full4k-pass >/dev/null 2>&1 || true
  systemctl stop 3proxy-s5-full4k-ip-port >/dev/null 2>&1 || true
  ...
  rm -f "${WORKDATA_PASS}" \
        "${WORKDATA_IP_PORT}" \
        "${WORKDIR}/proxy_s5_full4k_pass.txt" \
        "${WORKDIR}/proxy_s5_full4k_ip_port.txt" \
        "${BOOT_SCRIPT}" \
        "${PROXY_CFG_PASS}" \
        "${PROXY_CFG_IP_PORT}"
}
...
main() {
  ...
  echo "[*] SOCKS5 pass ports: ${FIRST_PORT_PASS}-${LAST_PORT_PASS} (${COUNT} proxies)"
  echo "[*] SOCKS5 ip:port ports: ${FIRST_PORT_IP_PORT}-${LAST_PORT_IP_PORT} (${COUNT} proxies)"
  ...
  echo "[*] Starting s5 full4k services..."
  systemctl daemon-reload
  systemctl enable --now 3proxy-s5-full4k-pass
  systemctl enable --now 3proxy-s5-full4k-ip-port

  echo "[*] Done."
  echo "[*] SOCKS5 pass list: ${WORKDIR}/proxy_s5_full4k_pass.txt"
  echo "[*] SOCKS5 no-auth list: ${WORKDIR}/proxy_s5_full4k_ip_port.txt"
  echo "[*] SOCKS5 pass count: $(wc -l < "${WORKDIR}/proxy_s5_full4k_pass.txt")"
  echo "[*] SOCKS5 no-auth count: $(wc -l < "${WORKDIR}/proxy_s5_full4k_ip_port.txt")"
}
