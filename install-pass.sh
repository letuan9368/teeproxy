#!/usr/bin/env bash
# Bootstrap: tai day du script + chay HTTP co pass.
# One-liner:
#   wget -qO- https://raw.githubusercontent.com/letuan9368/teeproxy/master/install-pass.sh | bash
set -euo pipefail
...
teeproxy_fetch "Tee-Proxy-Pass.sh"

export TEEPROXY_IP_MODE="${TEEPROXY_IP_MODE:-ipv4}"
exec bash "${DIR}/Tee-Proxy-Pass.sh" "$@"
